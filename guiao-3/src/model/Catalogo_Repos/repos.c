 ...
 * @file repos.c
 * @author grupo11
 * @brief Ficheiro que contém as funções que para trabalhar com os repos
 * @version 1.0
 * @date 2022-02-07
 * 
...
LRepos GGetRepo(int key, GHashTable *hash) {
    LRepos aux = (LRepos)g_hash_table_lookup(hash, &key);
...
int verica_commits_repo(LRepos repo, GHashTable *commits) {
    ...
    if(GContainsKey(repo->id, commits)) return 1;
    else return 0;
/*
...
int valid_Repos(LRepos r){
    ...
    if(r->id < 0 || r->owner_id < 0 || r->forks_count < 0 || r->open_issues < 0 || r->stargazers_count < 0 || r->size < 0)
        valid = 0;
    if(valid && (strcmp(r->full_name, "") == 0 || strcmp(r->license, "") == 0 || strcmp(r->description, "") == 0 ||
